...
class Environment:
    def __init__(self, boundary_coords, obstacles_coords_list=None):
        ...
        # Uçulabilir alan başlangıçta ana poligonun tamamıdır
        self.navigable_area = self.polygon
        self.obstacles = []
        
        # Engelleri ana alandan çıkar (Delikli poligon oluştur)
        ...
        # Başlangıç ve toplanma noktası olarak her zaman geçerli (engelsiz) bir nokta bul
...
class BaseSolver:
    ...
    def draw(self, ax, title):
        ...
        # Engelleri Çiz (Taralı ve Siyah)
        for obs in self.env.obstacles:
            ax.fill(*obs.exterior.xy, color='black', alpha=0.7, zorder=6, hatch='//')
            ax.plot(*obs.exterior.xy, color='red', linewidth=1.5, zorder=7)
        ...
        for i in range(self.drone_count):
            ...
            try:
                ...
                # Sadece engelsiz alan (navigable_area) ile kesişim al
                kirpilmis_hucre = self.env.navigable_area.intersection(hucre_poligonu)
...
class APFSolver(BaseSolver):
    ...
    def step(self):
        ...
        # KATI SINIR VE ENGEL KONTROLÜ (Ajan dışarı çıkarsa veya engele girerse yörüngeye geri oturt)
        for i in range(self.drone_count):
...
class LloydSolver(BaseSolver):
    def step(self):
        ...
        for i in range(self.drone_count): 
            ...
            try:
                hucre_poligonu = Polygon(hucre_kose_noktalari).buffer(0)
                # Voronoi hücresini engelsiz alana göre kesiyoruz
                kirpilmis_hucre = self.env.navigable_area.intersection(hucre_poligonu)
                
                if kirpilmis_hucre.area > 0:
                    sentroid = kirpilmis_hucre.centroid
                    yeni_hedef = np.array(list(sentroid.coords)[0])
                    
                    # Eğer hesaplanan ağırlık merkezi U şeklinde bir hücreden dolayı engelin içine düşerse,
                    # onu engelin dışına (en yakın geçerli sınıra) yapıştırıyoruz.
                    p_point = Point(yeni_hedef)
                    if not self.env.navigable_area.contains(p_point):
                        nearest_pt = self.env.navigable_area.boundary.interpolate(self.env.navigable_area.boundary.project(p_point))
                        yeni_hedef = np.array(list(nearest_pt.coords)[0])
                        
                    yeni_konumlar.append(yeni_hedef)
                else:
                    yeni_konumlar.append(self.positions[i])
            except Exception:
                yeni_konumlar.append(self.positions[i])
...
class SimulationManager:
        ...
        def on_start_clicked(event):
            try:
                ...
                if MIN_DRONE_SAYISI <= val <= MAX_DRONE_SAYISI:
                    ...
                    self.ax_setup.set_title(f"HATA: Lütfen {MIN_DRONE_SAYISI} ile {MAX_DRONE_SAYISI} arası değer girin!", color='red')
                    self.fig.canvas.draw_idle()
                    return
            except ValueError:
            ...
            if len(koseler) >= MIN_KOSSE_SAYISI:
                ...
                # Kullanıcıya dış sınırı çizerek göster
                self.ax_setup.plot(*Polygon(self.boundary_coords).exterior.xy, color='black', linewidth=3)
                self.fig.canvas.draw_idle()
                
                # ENGELLERİ (OBSTACLES) TANIMLAMA DÖNGÜSÜ
                engel_sayaci = 1
                while True:
                    self.ax_setup.set_title(f"ADIM 4: {engel_sayaci}. Engeli Çizin\n(İstemiyorsanız veya bitirmek için boş alanda doğrudan ENTER'a basın)", fontsize=14, color='blue')
                    self.fig.canvas.draw_idle()
                    
                    try:
                        engel_koseler = plt.ginput(n=-1, timeout=0, show_clicks=True, mouse_add=1, mouse_pop=3, mouse_stop=2)
                    except Exception:
                        break
                        
                    # ENTER'a basılmışsa veya iptal edilmişse döngüden çık
                    if len(engel_koseler) < MIN_KOSSE_SAYISI:
                        break
                        
                    self.obstacles_coords_list.append(np.array(engel_koseler))
                    
                    # Çizilen engeli anında ekrana yansıt
                    engel_poly = Polygon(engel_koseler)
                    self.ax_setup.fill(*engel_poly.exterior.xy, color='gray', alpha=0.5, hatch='//')
                    self.ax_setup.plot(*engel_poly.exterior.xy, color='red', linewidth=2)
                    self.fig.canvas.draw_idle()
    ...
    def start_simulation(self):
        ...
        # Dronları güvenli alana yerleştirmeye çalış, aksi halde en güvenli merkeze koy
        initial_positions = []
        gurultu_miktari = 0.02 if max(env.polygon.bounds) <= 1.0 else 2.0
        
        for _ in range(self.drone_count):
            pt = env.centroid + (np.random.rand(2) - 0.5) * gurultu_miktari
            if env.navigable_area.contains(Point(pt)):
                initial_positions.append(pt)
            else:
                initial_positions.append(env.centroid)
